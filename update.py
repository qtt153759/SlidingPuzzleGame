...
class Node:# luu thong tin trang thai(bảng,nút cha,heuristic,độ sâu trong cây tìm kiếm)
  ...
  def generate_children(self):#phát sinh nút con của nút đang sét trường hợp dùng heuristic(A* và human)
    ...
    val_list = [[x+1, y,"DOWN"],[x, y-1,"LEFT"],[x, y+1,"RIGHT"],[x-1, y,"UP"]] # các cách di chuyển,(nhận thấy nên ưu tiên theo thứ tự(xuống,trái,phải,lên)
    children = []
    for child in val_list:
      if child[0] >=0 and child [0] < n and child[1] >= 0 and child[1] < n:#ở biên thì loại bỏ một số trường hợp
        ...
        child_node = Node(child_board, self, node_manhattan(child_board), self.depth + 1,child[2])
  ...
  def generate_children_non_heuristic(self):#phát sinh nút con của nút đang sét trường dùng bfs(heuristic=0)
    for i in range(n):
      for j in range(n):
        if self.board[i][j] == 0:
          x, y = i, j
    val_list = [[x+1, y,"DOWN"],[x, y-1,"LEFT"],[x, y+1,"RIGHT"],[x-1, y,"UP"]]#bfs thì đi hướng nào cx không có nhiều khác biệt như human
    children = []
    for child in val_list:
      if child[0] >=0 and child [0] < n and child[1] >= 0 and child[1] < n:
        ...
        child_board[x][y] = child_board[child[0]][child[1]]
        child_board[child[0]][child[1]] = 0
        child_node = Node(child_board, self, 0, self.depth + 1,child[2])#heuristic=0
        children.append(child_node)
    return children
...
def row_col_goal_states(n):#xuất ra trạng thái đích theo các lớp layer(layer 0: row 0 và colum 0
  ...
  for layer in range(n-1):#xét layer
    ...
    for i in range(n - layer):#xét hàng
      goal_states.append(set([n * layer + i +layer+ 1]))#goal states có dạng [{1},{1,2},{1,2,3}]
      goal_states_point.append(n * layer + i +layer+ 1)#goal states point có dạng [{1},{2},{3}] áp dụng cho hàm FIND ở dưới
...
root = Node(board, None, manhattan(board, goal_states[curr_goal]), 0,"Start")#khởi tạo node đề bài và tính heuristic
...
if(FLAG=='HUMAN'):#Cách giải human
  ...
  f.write('\n')
  h_scale_factor = 5#chọn scale=3 vì 3*h(x)+g(x) sẽ giải nhanh hơn h(x)+g(x) như bình thường (ưu tiên heuristic)
  pq.heappush(h, (root.depth + h_scale_factor * root.heuristic, root))#push phần tử đầu tiên vào hàng đợi ưu tiên
  node = pq.heappop(h)[1]
  intentional_direction = FIND(root.board, goal_states_point[curr_goal])#tìm vị trí số 1 ở đâu rồi chuyền ô số 0 về đấy
  ...
  SAVE(node)
  node = Node(node.board, None, manhattan(board, goal_states[curr_goal]), 0,"da thay cai dau")
  pq.heappush(h,(node.depth+h_scale_factor*manhattan(node.board, goal_states[curr_goal]), node))
  ...
  while len(h) > 0:#dừng lại khi tìm thấy đích cuối cùng (curr_goal == len(goal_states)) hoặc heap hết
    ...
    if (int(goal_states_point[curr_goal]/n)+2 <=n and goal_states_point[curr_goal]%n==0 and node.board[int(goal_states_point[curr_goal]/n)][n-1]==goal_states_point[curr_goal]) :
      ...
      if node.board[int(goal_states_point[curr_goal]/n)+1][n-1]== 0:
        ...
        f.write('\n')
        SAVE(node)
        node = Node(node.board, None, manhattan(board, goal_states[curr_goal]), 0,"nut corner phai")
        ...
        node = node.generate_children_intentionally("DOWN")
        SAVE(node)
        node = Node(node.board, None, manhattan(board, goal_states[curr_goal]), 0,node.direction)#phai khoi tao lai node de cat duoi cho previous=0 ko no bi trung
    ...
    # Trường hợp xét ô góc dưới
      ...
      if node.board[n-1][goal_states_point[curr_goal]%n+1]== 0:
        ...
        f.write('\n')
        SAVE(node)
        node = Node(node.board, None, manhattan(board, goal_states[curr_goal]), 0,"corner trai")
    ...
    if isGoal(node.board, goal_states[curr_goal]) :#nếu đúng trạng thái đích thì chúng ta phải xét cả vòng lặp while
      ...
      node = Node(node.board, None, manhattan(board, goal_states[curr_goal]),0,"goal state hoan thanh")#mục đích của bước này là xóa previous nút cha
                                                                                # vì chúng ta đã lưu ở SAVE rồi, không thì sẽ bị trùng
      intentional_direction=FIND(node.board,goal_states_point[curr_goal])#tìm vị trí nút cần tìm goal_state_point[cur_goal] ở đâu rồi chuyền ô số 0 về đấy
      f.write("Bat dau tim kiem ")
      f.write(str(goal_states_point[curr_goal]))
      f.write('\n')
      if(intentional_direction[1]>0):
      ...
      node = Node(node.board, None, manhattan(board, goal_states[curr_goal]), 0," goal state da thay")
